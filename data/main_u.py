...
"""USE CLEANED AND PREPED DATASET INSTEAD"""
if os.path.exists("prep.csv"):
    print("File already exists. Skipping extraction.")
else:
    with zipfile.ZipFile("data/prep.zip", 'r') as zip_ref:
        zip_ref.extract("prep.csv", path="./")
        print("extracted")

df = pd.DataFrame(pd.read_csv("prep.csv"))
