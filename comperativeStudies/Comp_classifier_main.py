import argparse
import os
import sys

import pandas as pd
import torch
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import utils
from RLGAN import MNISTClassifier
from AEDQN import classifiers
from comperativeStudies import MNISTClassifier_trainer


def parse_args(args):
    parser = argparse.ArgumentParser(description='Select which model to run: RL-GAN or AE+DQN [REQUIRED]')

    parser.add_argument('--model', choices=['RL-GAN', 'AE+DQN'], required=True)

    # batch sizes and epochs
    parser.add_argument('--batch_size_train', default=32, type=int)
    parser.add_argument('--batch_size_test', default=64, type=int)
    parser.add_argument('--numEpochs', default=51, type=int)

    # when the discriminator loss reaches a threshold, we save the clf state dictionary
    parser.add_argument("--loss_threshold", default=0.6, type=float)


    # if test: ---train False
    parser.add_argument('--train', action='store_true')
    # unaug = unaugmented dataset = original dataset : if False then augmented dataset
    parser.add_argument("--unaug_dataset", default=True)
    # Generate labels for synthetic dataset
    parser.add_argument("--label_gen", action="store_true")
    parser.add_argument("--synth_dataset", default=False)
    parser.add_argument("--synth_dataset_path", default="results/RL-GAN/rl_ds1.csv") # path to unsupervised dataset
    # PLEASE USE THE ABSOLUTE PATH IF YOU GET A "NO FILE IS FOUND" ERROR!!!
    # save labels
    parser.add_argument("--labels_file", default = "results/RL-GAN/labels1.csv")
    # save classifier state dictionary
    parser.add_argument("--save_state_dict", default="results/RL-GAN/clf1.pth")

    return parser.parse_args(args)

if __name__ == "__main__":
    args = sys.argv[1:]
    args = parse_args(args)

    print("using", args.unaug_dataset)
    original_ds = utils.dataset(original=args.unaug_dataset, train=args.train)
    train_loader, val_loader = utils.dataset_function(original_ds, args.batch_size_train, args.batch_size_test, train=True)
    test_loader = utils.dataset_function(original_ds, args.batch_size_train, args.batch_size_test, train=False)

    if args.model == "RL-GAN":
        print("using RL-GAN")
        classifier = MNISTClassifier.Classifier()
    else:
        print("using AE+DQN")

        classifier = classifiers.Classifier(30, 4)

    optimizer = torch.optim.SGD(classifier.parameters(), lr=0.001)

    if args.train:
        for epoch in range(args.numEpochs):
            # classifier pre-training
            train_loss = MNISTClassifier_trainer.classifier_train(classifier, train_loader, optimizer)
            # evaluation
            if epoch % 10 == 0:
                val_loss = MNISTClassifier_trainer.classifier_val(classifier, val_loader)
                best_val_loss = args.loss_threshold
                if val_loss < best_val_loss:
                    best_val_loss = val_loss
                    torch.save(classifier.state_dict(), f'{args.save_state_dict}')

    else:
        # testing
        MNISTClassifier_trainer.classifier_test(classifier, args.save_state_dict, test_loader)

    if args.label_gen:
        # generate labels
        print("gen labels")
        labels = MNISTClassifier_trainer.gen_labels(classifier, args.save_state_dict, args.synth_dataset_path, args.batch_size_train)
        labels = labels.detach().cpu().numpy()
        labels = pd.DataFrame(labels, columns=["attack_cat"])
        labels.to_csv(f"{args.labels_file}", index=False)
